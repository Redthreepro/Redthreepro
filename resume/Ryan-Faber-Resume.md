...
- **Built and run a production outreach automation on self-hosted n8n and Google Apps Script**: property-listing ingest qualified against a locally hosted 39-county state well-records dataset, inspector drive-time qualification, lead routing, and a guardrail layer (send window, per-run and daily caps, per-domain limit, 7-day cooldown, ramp-up mode, kill switch, test and dry-run modes) with persistent state and failure logging, all running from and controlled by a spreadsheet the office already uses, with a kill switch in its menu. 4,019 emails sent January to August 2026 at 95% delivery, 2% hard bounce, and zero spam complaints. The v1 pipeline was built with the guardrails first and deliberately held back until the compliance posture was right.
