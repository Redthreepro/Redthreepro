...
- **Built and run production n8n/API automations on self-hosted infrastructure**: property-listing ingest with service-area and well/septic rules, do-not-contact and cooldown controls, inspector drive-time qualification, lead routing, outreach, and persistent state with duplicate protection and failure logging. An earlier Node pipeline for the same problem was built with full guardrails and deliberately not launched until the trigger and compliance posture were right.
